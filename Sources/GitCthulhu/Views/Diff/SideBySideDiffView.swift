...
struct SideBySideDiffView: View {
    ...
    private func hunkContent(for hunk: DiffHunk) -> some View {
        VStack(spacing: 0) {
            ...
            // Process lines for side-by-side display
            let lineGroups = processlinesForSideBySide(hunk.lines)
            
            ForEach(lineGroups.indices, id: \.self) { groupIndex in
                let group = lineGroups[groupIndex]
                sideBySideLineView(for: group)
            }
        }
    }
    ...
    // MARK: - Side-by-Side Line View
    
    private func sideBySideLineView(for group: LineGroup) -> some View {
        HStack(spacing: 0) {
            // Old (left) side
            sideContent(for: group.oldLine, isOldSide: true)
            ...
            // New (right) side
            sideContent(for: group.newLine, isOldSide: false)
        }
        .frame(maxWidth: .infinity)
    }
    ...
    private func sideContent(for line: DiffLine?, isOldSide: Bool) -> some View {
        HStack(spacing: 0) {
            if let line = line {
                // Line type indicator (only for non-context lines)
                ...
                // Line number
                if showLineNumbers {
                    lineNumber(for: line, isOldSide: isOldSide)
                }
    ...
    private func lineNumber(for line: DiffLine, isOldSide: Bool) -> some View {
        let lineNumber = isOldSide ? line.oldLineNumber : line.newLineNumber
        
        return Text(lineNumber?.description ?? "")
            .font(.system(.caption, design: .monospaced))
            .foregroundColor(.secondary)
    ...
    // MARK: - Empty Line Placeholder
    
    private var emptyLinePlaceholder: some View {
        HStack {
            if showLineNumbers {
                Spacer()
                    .frame(width: 20 + 40 + 16) // indicator + line number + padding
            } else {
                Spacer()
                    .frame(width: 20) // just indicator
            }
    ...
    // MARK: - Line Processing for Side-by-Side
    ...
    private func processlinesForSideBySide(_ lines: [DiffLine]) -> [LineGroup] {
        ...
        while i < lines.count {
            ...
            case .context:
                ...
                groups.append(LineGroup(oldLine: currentLine, newLine: currentLine))
                i += 1
            ...
            case .removed:
                ...
                // Find the next added line (if any)
                while j < lines.count && lines[j].type == .removed {
                    j += 1
                }
                
                if j < lines.count && lines[j].type == .added {
                    addedLine = lines[j]
                }
                
                if let addedLine = addedLine {
                    // Pair removed and added lines
                    groups.append(LineGroup(oldLine: currentLine, newLine: addedLine))
                    i = j + 1
                } else {
                    // Removed line without corresponding added line
                    groups.append(LineGroup(oldLine: currentLine, newLine: nil))
                    i += 1
                }
                
            case .added:
                // Check if this added line was already paired with a removed line
                let wasPaired = i > 0 && lines[i - 1].type == .removed
                
                if !wasPaired {
                    // Added line without corresponding removed line
                    groups.append(LineGroup(oldLine: nil, newLine: currentLine))
                }
                i += 1
                
            case .noNewlineAtEnd:
                // Special case for no newline indicator
                groups.append(LineGroup(oldLine: currentLine, newLine: currentLine))
                i += 1
            }
        }
