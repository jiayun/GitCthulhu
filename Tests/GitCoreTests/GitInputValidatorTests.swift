...
struct GitInputValidatorTests {
    ...
    func unicodeHandling() async throws {
        ...
        for input in unicodeInputs {
            // Should handle Unicode correctly without throwing
            let sanitized = try GitInputValidator.sanitizeInput(input)
            #expect(!sanitized.isEmpty)
        }
    }

    @Test("Mixed dangerous character combinations")
    func mixedDangerousCharacterCombinations() async throws {
        let dangerousInputs = [
            "normal;evil",
            "test|pipe",
            "cmd&background",
    ...
    func whitespaceNormalization() async throws {
        ...
        for input in inputsWithWhitespace {
            let sanitized = try GitInputValidator.sanitizeInput(input)
            #expect(sanitized == input.trimmingCharacters(in: .whitespacesAndNewlines))
